...
* Estimamos una función *time-varying risk aversion* (TVRA) siguiendo los parametros de @bollerslev2011dynamic.

* 8 países (Francia, Alemania, Reino Unido, China, Japón, Suiza, Estados Unidos y Corea del Sur).

* Estudiamos si la variable TVRA puede predecir el retorno accionario *in-sample*. 
...
\begin{equation}
\begin{array}{c}
dp_{t}=\mu_{t}()dt+\sqrt{V_{t}}dB_{1t}\\
dV_{t}=\kappa(\theta-V_{t})dt+\sigma_{t}()dB_{2t}
\end{array}
\end{equation}

* @Bollerslev2002 muestran que la distribución *risk-neutral* está dado por:
...
* @britten2000option prueban que la medida de volatilidad puede ser computada como el promedio de un continuo de *$\Delta$-maturity options*.

$$
IV^{*}_{t,t+\Delta}=2\int\frac{C(t+\Delta,K)-C(t+\Delta)}{K^{2}}dK
$$

* Donde $C(t+\Delta,K)$ es el precio de una opción Europea con madurez al tiempo $t$ con precio *strike* $K$, es igual a la expectativa real de neutralidad de riesgo de la volatilidad integrada 
...
* Consideramos los momentos definidos en (3) y (5), y el rezago de la volatilidad realizada como instrumento adicional.
